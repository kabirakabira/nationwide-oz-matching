...
### Clean up
tracts.nationwide <- tracts.nationwide %>%
  `colnames<-`(c("GEOID", "geometry", "Population", "PovertyPopulation", "MHHI")) %>%
  mutate(Pov_Rate = PovertyPopulation / Population) %>%
  select(GEOID, Pov_Rate, MHHI, geometry)
...
st_write(tracts.nationwide,
          "data/processed/Tracts_OZdesignation_LimitedData.shp")
